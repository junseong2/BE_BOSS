...
public interface SettlementRepository extends JpaRepository<Settlement, Long> {
    ...
    //ㅁㅋ
    @Query("SELECT new com.onshop.shop.settlement.SettlementsDTO("
            + " st.settlementId, st.createdDate, st.updatedDate, st.status, "
            + " '우리은행', u.username, '1234567866', st.requestedAmount) "
            + "FROM Settlement st "
            + "JOIN Seller s ON st.seller.sellerId = s.sellerId "
            + "JOIN User u ON s.userId = u.userId "
            + "WHERE st.createdDate BETWEEN :startDate AND :endDate "
            + "AND (:settlementId IS NULL OR st.settlementId = :settlementId) "
            + "AND (:username IS NULL OR u.username LIKE %:username%)")
    Page<SettlementsDTO> findAllByAdmin(
        @Param("startDate") LocalDateTime startDate, 
        @Param("endDate") LocalDateTime endDate,
        @Param("settlementId") Long settlementId,  
        @Param("username") String username,
