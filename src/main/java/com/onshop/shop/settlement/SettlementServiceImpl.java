...
public class SettlementServiceImpl implements SettlementService {
	...
	public void updateSellerStatus(Long sellerId, String status) {
        ...
        if (sellerOptional.isPresent()) {
            ...
            seller.setRegistrationStatus(status);  // 상태 변경
	 ...
	 @Override
	    public SettlementResponseDTO getAllSettlements(int page, int size, SettlementSearchConditionDTO condition) {
	        Pageable pageable = PageRequest.of(page, size);

	        List<SettlementsDTO> settlements = settlementRepository.findAllByAdmin(
	                condition.getStartDate(),
	                condition.getEndsDate(),
	                condition.getSettlementId().orElse(null),
	                condition.getUsername(),
	        ...
	        Long totalCount = settlementRepository.countByAdmin(
	                condition.getStartDate(),
	                condition.getEndsDate(),
	                condition.getSettlementId().orElse(null),
	                condition.getUsername()
